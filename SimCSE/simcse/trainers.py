...
try:
    ...
    try:
        # newer versions expose unwrap_model
        from transformers.modeling_utils import unwrap_model as _model_unwrap
    except Exception:
        # minimal fallback: strip common wrappers
...
class CLTrainer(Trainer):
    ...
    @property
    def local_rank(self):
        return getattr(self.args, "local_rank", -1)

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # 兼容：某些版本 Trainer 不一定预先挂这些属性
        if not hasattr(self, "deepspeed"):
            self.deepspeed = None
                    # transformers 版本差异：旧代码用 use_amp，新版 Trainer 用 use_cuda_amp/use_cpu_amp
        if not hasattr(self, "use_amp"):
            self.use_amp = bool(getattr(self, "use_cuda_amp", False) or getattr(self, "use_cpu_amp", False))
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        self.state.eval_steps = self.args.eval_steps if self.args.eval_steps is not None else self.args.logging_steps
